...
def novo_jogo(request: Request, nome: str = Form(...), desenvolvedora: str = Form(""),
              data_lancamento: str = Form(""), descricao: str = Form(""),
              lore: str = Form(""), genero: str = Form(""), capa_url: str = Form("")):
    admin = _check_admin(request)
    if not admin:
        return RedirectResponse("/", status_code=302)
    execute(
        ...
        (nome, desenvolvedora, data_lancamento or None, descricao, lore, genero, capa_url)
    )
...
def editar_jogo(request: Request, jogo_id: int, nome: str = Form(...),
                desenvolvedora: str = Form(""), data_lancamento: str = Form(""),
                descricao: str = Form(""), lore: str = Form(""),
                genero: str = Form(""), capa_url: str = Form("")):
    admin = _check_admin(request)
    if not admin:
        return RedirectResponse("/", status_code=302)
    execute(
        ...
        (nome, desenvolvedora, data_lancamento or None, descricao, lore, genero, capa_url, jogo_id)
    )
    return RedirectResponse("/admin", status_code=302)
...
def toggle_admin(request: Request, uid: int):
    ...
    if usuario and usuario["id"] != admin["id"]:
        execute("UPDATE usuarios SET is_admin=%s WHERE id=%s", (not usuario["is_admin"], uid))
    return RedirectResponse("/admin", status_code=302)
