"""
routers/jogos_router.py — Biblioteca, detalhes do jogo, reviews.
"""
from fastapi import APIRouter, Request, Form, Depends
from fastapi.responses import HTMLResponse, RedirectResponse
from fastapi.templating import Jinja2Templates
from database import query_one, query_all, execute
from deps import get_usuario_atual, requer_login
...
def detalhe_jogo(request: Request, jogo_id: int):
    ...
    reviews = query_all(
        """SELECT r.*, u.username, u.foto_url
           FROM reviews r JOIN usuarios u ON r.usuario_id = u.id
           WHERE r.jogo_id = %s ORDER BY r.criado_em DESC""",
        (jogo_id,)
    )
    minha_review = None
    meu_status = None
...
def salvar_review(
    ...
    if existente:
        execute(
            "UPDATE reviews SET nota=%s, comentario=%s WHERE id=%s",
            (nota, comentario, existente["id"])
        )
    else:
        execute(
            "INSERT INTO reviews (usuario_id, jogo_id, nota, comentario) VALUES (%s,%s,%s,%s)",
            (usuario["id"], jogo_id, nota, comentario)
        )
    # Atualiza média no jogo
...
def favoritar(request: Request, jogo_id: int):
    ...
    if fav:
        ...
        execute("INSERT INTO favoritos (usuario_id, jogo_id) VALUES (%s,%s)", (usuario["id"], jogo_id))
    return RedirectResponse(f"/jogo/{jogo_id}", status_code=302)
...
def atualizar_status(request: Request, jogo_id: int, status: str = Form(...)):
    ...
    if existente:
        ...
        execute(
            "INSERT INTO status_jogo (usuario_id, jogo_id, status) VALUES (%s,%s,%s)",
